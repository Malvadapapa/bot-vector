...
class BanWarningSystem {
     ...
     */
    addInfraction(userId, username, type, description, severity) {
        ...
        const recentInfractions = this.banData.infractions.filter(i => i.userId === userId && i.date >= sevenDaysAgo);
     ...
     */
    getRestrictionMinutesRemaining(userId) {
        const restriction = this.restrictedUsers.get(userId);
        if (!restriction)
            return 0;
        const now = new Date();
     ...
     */
    adminBan(userId, username, reason) {
...
exports.BanWarningSystem = BanWarningSystem;
