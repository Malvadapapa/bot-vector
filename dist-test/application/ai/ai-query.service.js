...
class AIQueryService {
    constructor(aiProvider, rateLimitService, knowledgeContextService, moderationRepository, ragQueryService) {
    ...
    async answer(userId, prompt, now, isAdmin = false) {
    ...
    async generateAnswer(userId, prompt, now, isAdmin) {
        ...
        const aiText = await this.withTimeout(this.aiProvider.generateContent(userId, mergedPrompt), AIQueryService.RESPONSE_TIMEOUT_MS);
    ...
    async classifyOffTopicWithAI(userId, prompt) {
        ...
        try {
            const result = await this.withTimeout(this.aiProvider.generateContent(userId, classifierPrompt), AIQueryService.TOPIC_CLASSIFICATION_TIMEOUT_MS);
...
AIQueryService.RESPONSE_TIMEOUT_MS = 25000;
AIQueryService.TOPIC_CLASSIFICATION_TIMEOUT_MS = 8000;
// Palabras clave que indican contenido académico del ISPC
AIQueryService.ACADEMIC_KEYWORDS = [
...
AIQueryService.GREETING_PATTERNS = [
...
AIQueryService.INFRACTION_THRESHOLD = 5;
