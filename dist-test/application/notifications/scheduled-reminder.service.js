...
exports.ScheduledReminderService = void 0;
class ScheduledReminderService {
     ...
     */
    createRemindersForExam(examId, examName, examDate, examTime, examType, endTime) {
        const reminders = [];
        const now = new Date();
        ...
        if (sevenDaysBefore > now) {
            reminders.push(this.createReminder(examId, examName, examDate, examTime, sevenDaysBefore, 'early-notice'));
        }
        ...
        if (oneHourBefore > now) {
            reminders.push(this.createReminder(examId, examName, examDate, examTime, oneHourBefore, 'hour-before'));
        }
        ...
        if (examType === 'franja' && endTime) {
            ...
            if (startMinus10 > now) {
                reminders.push(this.createReminder(examId, examName, examDate, examTime, startMinus10, 'franja-start'));
            }
            ...
            if (endMinus10 > now) {
                reminders.push(this.createReminder(examId, examName, examDate, endTime, endMinus10, 'franja-end'));
            }
        }
        return reminders;
    }
    ...
    getPendingReminders() {
        const now = new Date();
        return Array.from(this.reminders.values()).filter(r => !r.sent && r.reminderTime <= now && r.reminderTime > new Date(now.getTime() - 60000));
    }
    /**
    ...
    getUpcomingReminders(userId) {
        ...
        let reminders = Array.from(this.reminders.values()).filter(r => !r.sent && r.reminderTime >= now && r.reminderTime <= tomorrow);
        if (userId) {
     ...
     */
    cleanupOldReminders(daysToKeep = 30) {
     ...
     */
    createReminder(examId, examName, examDate, examTime, reminderTime, type) {
...
exports.ScheduledReminderService = ScheduledReminderService;
