...
const moderation_admin_command_service_js_1 = require("../moderation/moderation-admin-command.service.js");
const admin_logging_command_service_js_1 = require("../admin/admin-logging-command.service.js");
const dashboard_panel_service_js_1 = require("../admin/dashboard-panel.service.js");
const menu_persistence_service_js_1 = require("./menu-persistence.service.js");
...
const MODULE_DIR = path.dirname((0, url_1.fileURLToPath)(import.meta.url));
class AcademicCalendarService {
    constructor(dynamicMessageService, reminderRepository, managedClassRepository, managedTeacherRepository, userProfileRepository, classCommissionScheduleRepository, commissionRepository, groupContextRepository, examMenuService, editExamMenuService, removeNotificationMenuService, managedExamRepository, loggingService) {
        ...
        this.classCommissionScheduleRepository = classCommissionScheduleRepository;
        this.commissionRepository = commissionRepository;
        this.groupContextRepository = groupContextRepository;
        this.examMenuService = examMenuService;
        this.editExamMenuService = editExamMenuService;
        this.removeNotificationMenuService = removeNotificationMenuService;
        this.managedExamRepository = managedExamRepository;
        this.loggingService = loggingService;
        this.menuStateByUser = new Map();
        this.moderationAdminService = new moderation_admin_command_service_js_1.ModerationAdminCommandService();
        this.menusPath = this.resolveDataFilePath('menus.json');
        // Inicializar servicios
        if (this.loggingService) {
            this.adminLoggingService = new admin_logging_command_service_js_1.AdminLoggingCommandService(this.loggingService);
        }
        // Inicializar servicio de persistencia si tenemos los repositorios necesarios
        // Inicializar servicio de panel
        if (this.loggingService) {
            this.dashboardPanelService = new dashboard_panel_service_js_1.DashboardPanelService(undefined, this.loggingService);
        }
        if (this.examMenuService && this.managedExamRepository) {
            this.menuPersistenceService = new menu_persistence_service_js_1.MenuPersistenceService(this.examMenuService, this.managedExamRepository, this.loggingService);
        }
    }
    hasActiveMenuState(userId) {
        return this.menuStateByUser.has(userId);
    }
    clearMenuState(userId) {
        this.menuStateByUser.delete(userId);
    }
    setNotificationSender(sender) {
        this.menuPersistenceService?.setNotificationSender(sender);
    }
    async handleCommand(userId, commandText, now, isAdmin = false, groupId) {
    ...
    async handleMenuInput(userId, rawText, groupId) {
    ...
    async formatDay(currentDt, groupId) {
    ...
    async formatCurrentClassLink(userId, currentDt, groupId) {
        const dayName = DAY_NAMES[currentDt.getDay()] || 'dia';
        ...
        const available = classes
            ...
            .sort((a, b) => a.start - b.start); // la mas proxima primero
        if (!available.length) {
            return 'En este momento no hay ninguna clase en curso 🙂';
        }
        const currentClass = available[0];
        const minutesUntilStart = currentClass.start - nowMinutes;
    ...
    async formatManagedExams(userId, groupId) {
    ...
    async formatWeekEvents(userId, currentDt, offsetDays) {
        ...
        for (let i = 0; i < 7; i += 1) {
            ...
            dayExams.forEach((e) => {
                const time = (e.horaInicio || e.exam_time || '');
                const text = `Examen ${time ? time + ' ' : ''}${e.subject} (${e.exam_type})`;
                structured.push({ time: time || null, text });
            });
            dayNotices.forEach((n) => {
                const title = String(n.title || 'Aviso');
                const m = title.match(/^(\d{1,2}:\d{2})\s*(.*)$/);
                if (m)
                    structured.push({ time: m[1], text: `Aviso ISPC: ${m[2]}` });
                else
                    structured.push({ time: null, text: `Aviso ISPC: ${title}` });
            });
            reminders.forEach((r) => {
                const textRaw = String(r);
                const m = textRaw.match(/^(\d{1,2}:\d{2})\s*(.*)$/);
                if (m)
                    structured.push({ time: m[1], text: m[2] });
                else
                    structured.push({ time: null, text: textRaw });
            });
    ...
    async resolveTemplate(template, node, userId) {
    ...
    formatHolidays(node) {
        ...
        if (!holidays.length)
            return 'Feriados: no hay feriados cargados.';
    ...
    async getClassesForWeekday(dayName, userId, groupId) {
    ...
    async getUserCommissionId(userId, groupId) {
    ...
    filterClassesByCommission(classes, userCommissionId) {
    ...
    filterExamsByCommission(exams, userCommissionId) {
