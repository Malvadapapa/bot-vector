...
exports.SystemCoordinatorService = void 0;
const case_escalation_service_js_1 = require("../admin/case-escalation.service.js");
class SystemCoordinatorService {
    constructor(conversationAnalysis, caseEscalation, scheduledReminders, dashboardPanel, banSystem) {
        this.conversationAnalysis = conversationAnalysis;
        this.caseEscalation = caseEscalation;
     ...
     */
    async generateUserFullReport(userId, username) {
     ...
     */
    async getSystemExecutiveSummary() {
        ...
        const criticalCases = this.caseEscalation.getCasesByLevel(case_escalation_service_js_1.EscalationLevel.CRITICAL);
     ...
     */
    createAutoEscalatedCase(userId, username) {
        const pattern = this.conversationAnalysis.getPattern(userId);
        if (!pattern) {
            return null;
        }
        // Determinar nivel de escalado
        let level = case_escalation_service_js_1.EscalationLevel.LOW;
        let reason = '';
        if (pattern.suspicionScore >= 90) {
            level = case_escalation_service_js_1.EscalationLevel.CRITICAL;
            ...
            level = case_escalation_service_js_1.EscalationLevel.HIGH;
            reason = 'Ciclo de infracciones detectado';
        }
        else if (pattern.suspicionScore >= 60) {
            level = case_escalation_service_js_1.EscalationLevel.MEDIUM;
            reason = 'Comportamiento sospechoso';
        }
        else if (pattern.infractions > 0) {
            level = case_escalation_service_js_1.EscalationLevel.LOW;
            reason = 'Infracción registrada';
        }
        else {
            return null;
        }
        return this.caseEscalation.createCase(userId, reason, `Análisis automático detectó: ${pattern.messageCount} mensajes, ${pattern.infractions} infracciones, ${pattern.suspicionScore}/100 sospecha`, level, username);
    }
    /**
     * Obtiene usuarios que necesitan revisión manual
     */
    getUsersNeedingReview() {
        const review = [];
     ...
     */
    formatRecommendedAction(userId, analysis) {
...
exports.SystemCoordinatorService = SystemCoordinatorService;
