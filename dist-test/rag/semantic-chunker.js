...
class SemanticChunker {
    constructor(config = { targetChunkSize: 2000, overlapSize: 300 } // approx 500-700 tokens
    ) {
        this.config = config;
    }
    chunkText(pages, sourceFile, sourceHash) {
        const chunks = [];
        ...
        return chunks.map((chunk, index) => {
            const id = node_crypto_1.default.createHash('sha256').update(`${sourceHash}-${index}`).digest('hex');
            return {
                text: chunk.text,
                metadata: {
                    id,
