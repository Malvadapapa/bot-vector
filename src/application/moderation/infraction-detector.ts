...
export interface InfractionEvent {
  userId: string;
  username: string;
  type: 'off-topic' | 'spam' | 'privados-excesivos' | 'inyeccion' | 'lenguaje-ofensivo' | 'manipulacion-ia' | 'acceso-datos' | 'preguntas-repetidas' | 'malintencionadas' | 'automatizacion';
  severity: 'leve' | 'moderada' | 'grave';
  description: string;
  timestamp: Date;
}

export class InfractionDetector {
  ...
  private readonly AI_MANIPULATION_PATTERNS = [
  ...
  private readonly AUTOMATION_PATTERNS = [
    /bot\s+script/gi,
    /automation\s+tool/gi,
    /auto\s+sender/gi,
  ];

  private messagesByUser = new Map<string, { timestamp: Date; count: number }>();
  private privateMessagesByUser = new Map<string, { timestamps: Date[] }>();
  private recentQueriesByUser = new Map<string, string[]>();

  /**
   * Verifica si un mensaje contiene infracciones
   */
  detectInfraction(userId: string, username: string, message: string, messageType: 'group' | 'private' = 'group'): InfractionEvent | null {
  ...
  private _detectOffensiveLanguage(message: string): string | null {
  ...
  private _detectAIManipulation(message: string): boolean {
    return this.AI_MANIPULATION_PATTERNS.some(pattern => pattern.test(message));
  }

  private _detectCodeInjection(message: string): boolean {
    return this.SQL_INJECTION_PATTERNS.some(pattern => pattern.test(message));
  }

  private _detectSpam(userId: string, message: string, now: Date): InfractionEvent | null {
  ...
  private _detectPrivateSpam(userId: string, username: string, now: Date): InfractionEvent | null {
    const privateMessages = this.privateMessagesByUser.get(userId) || { timestamps: [] };

    // Limpiar timestamps antiguos (> 1 hora)
    privateMessages.timestamps = privateMessages.timestamps.filter(
      ts => now.getTime() - ts.getTime() < 3600000 // 1 hora
    );
  ...
  private _detectRepetition(userId: string, username: string, message: string, now: Date): InfractionEvent | null {
  ...
  private _detectAutomation(message: string): boolean {
  ...
  private _getEditDistance(s1: string, s2: string): number {
    const costs: number[] = [];
