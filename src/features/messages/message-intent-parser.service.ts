import { ParsedMessage } from './messages.models.js';
...
export class MessageIntentParserService {
  public parseMessage(rawText: string, referenceDt?: Date): ParsedMessage {
  ...
  private extractProbableDate(text: string, referenceDt?: Date): Date | null {
    ...
    if (byMonthName) {
      ...
      if (month) {
        ...
        if (dt < now) dt = new Date(year + 1, month - 1, day);
        return dt;
      }
    }
