...
import { ManagedExamRepository, ReminderRepository } from './academic-calendar.repository.js';

export interface RemoveNotificationState {
  userId: string;
  stage: 'selecting-kind' | 'selecting-exam' | 'confirming';
  kind?: 'reminder' | 'exam';
...
export class RemoveNotificationMenuService {
  private userStates = new Map<string, RemoveNotificationState>();
  ...
  async startRemovalFlow(userId: string): Promise<string> {
  ...
  private getAvailabilityText(exam: any): string {
